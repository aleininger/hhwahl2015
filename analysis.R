...
#   geom_smooth(method=lm, fill = 'steelblue2', color = 'dodgerblue4') +
#   geom_point(size = 3, color = 'lightsteelblue4') +
...
fDifToArbeit <- ggplot(d, aes(x = arbeitslosigkeit, y = (to-to11))) +
  ...
  ylab('Wahlbeteiligung 2015-2011 %-Punkte') + xlab('%')
...
fAfdAuslanderArbeit <- ggplot(NULL, aes(x = ausres, y = tores)) + geom_point(size = 3, color = 'lightsteelblue4') +
